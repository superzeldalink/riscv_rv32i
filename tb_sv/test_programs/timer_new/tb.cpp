...
const vluint64_t sim_duration = 100000; // Simulation duration in time units
...
int main(int argc, char **argv)
{
    ...
    while (!Verilated::gotFinish() && main_time < sim_duration)
    {
        ...
        if((main_time > 1000 && main_time <= 2000) || main_time > 5000 && main_time <= 6000) {
            top->io_keys_i = 0xFFFFFFFE;
        } else {
            top->io_keys_i = 0xFFFFFFFF;
        }
