...
module memory #(
  ...
  always @(posedge clk) begin
    if (write_en_i) begin
      casez (addr_i[1:0])
        2'b00:
        {mem[mem_addr + 0][3], mem[mem_addr + 0][2], mem[mem_addr + 0][1], mem[mem_addr + 0][0]} <= data_in_i;
        2'b01:
        {mem[mem_addr + 1][0], mem[mem_addr + 0][3], mem[mem_addr + 0][2], mem[mem_addr + 0][1]} <= data_in_i;
        2'b10:
        {mem[mem_addr + 1][1], mem[mem_addr + 1][0], mem[mem_addr + 0][3], mem[mem_addr + 0][2]} <= data_in_i;
        2'b11:
        {mem[mem_addr + 1][2], mem[mem_addr + 1][1], mem[mem_addr + 1][0], mem[mem_addr + 0][3]} <= data_in_i;
      endcase
    end
  end
